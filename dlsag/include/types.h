#ifndef TRILERO_DLSAG_INCLUDE_TYPES
#define TRILERO_DLSAG_INCLUDE_TYPES
...
typedef struct {
  ...
  ec_public_key_t ec_pk0;
  ec_secret_key_t ec_sk0;
  ec_public_key_t ec_pk1;
  ec_secret_key_t ec_sk1;
  bn_t m;
} keys_st;
...
#define keys_new(keys)                              \
  do {                                              \
    ...
    ec_public_key_new((keys)->ec_pk0);              \
    ec_secret_key_new((keys)->ec_sk0);              \
    ec_public_key_new((keys)->ec_pk1);              \
    ec_secret_key_new((keys)->ec_sk1);              \
    bn_new((keys)->m);                              \
  } while (0)
...
#define keys_free(keys)                             \
  do {                                              \
    ...
    ec_public_key_free((keys)->ec_pk0);             \
    ec_secret_key_free((keys)->ec_sk0);             \
    ec_public_key_free((keys)->ec_pk1);             \
    ec_secret_key_free((keys)->ec_sk1);             \
    bn_free((keys)->m);                             \
    free(keys);                                     \
    keys = NULL;                                    \
  } while (0)
...
typedef ring_st *ring_t;

#define ring_null(ring) ring = NULL;

#define ring_new(ring, ring_size)                     \
  do {                                                \
    ring = malloc(sizeof(ring_st));                   \
    if (ring == NULL) {                               \
      RLC_THROW(ERR_NO_MEMORY);                       \
    }                                                 \
    (ring)->pk_0 = malloc(ring_size * sizeof(ec_t));  \
    if ((ring)->pk_0 == NULL) {                       \
      RLC_THROW(ERR_NO_MEMORY);                       \
    }                                                 \
    ...
    (ring)->pk_1 = malloc(ring_size * sizeof(ec_t));  \
    if ((ring)->pk_1 == NULL) {                       \
      RLC_THROW(ERR_NO_MEMORY);                       \
    }                                                 \
    for (size_t i = 0; i < ring_size; i++) {          \
    ...
    (ring)->m = malloc(ring_size * sizeof(bn_t));     \
    if ((ring)->m == NULL) {                          \
      RLC_THROW(ERR_NO_MEMORY);                       \
    }                                                 \
    for (size_t i = 0; i < ring_size; i++) {          \
...
#define ring_free(ring, ring_size)                    \
  do {                                                \
    for (size_t i = 0; i < ring_size; i++) {          \
      ec_free((ring)->pk_0[i]);                       \
      ec_free((ring)->pk_1[i]);                       \
...
#endif // TRILERO_DLSAG_INCLUDE_TYPES