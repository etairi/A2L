...
int generate_keys_and_write_to_file(const cl_params_t params) {
	...
	bn_t q, x, y, m_alice_tumbler, m_bob_tumbler;
	bn_t ec_sk0_alice, ec_sk1_alice; 
	bn_t ec_sk0_bob, ec_sk1_bob;
	bn_t ec_sk0_tumbler, ec_sk1_tumbler;
	ec_t ec_pk0_alice, ec_pk1_alice; 
	ec_t ec_pk0_bob, ec_pk1_bob; 
	ec_t ec_pk0_tumbler, ec_pk1_tumbler;
	ec_t ec_pk0_alice_tumbler, ec_pk1_alice_tumbler; 
	ec_t ec_pk0_bob_tumbler, ec_pk1_bob_tumbler;
	bn_t ec_ring_m[RING_SIZE];
	ec_t ec_ring_pk_0[RING_SIZE], ec_ring_pk_1[RING_SIZE];

	ps_secret_key_t ps_sk_tumbler;
	ps_public_key_t ps_pk_tumbler;

	uint8_t serialized_m[RLC_BN_SIZE];
	...
	bn_null(x);
	bn_null(y);
	bn_null(m_alice_tumbler);
	bn_null(m_bob_tumbler);
	bn_null(ec_sk0_alice);
	bn_null(ec_sk1_alice);
	bn_null(ec_sk0_bob);
	bn_null(ec_sk1_bob);
	bn_null(ec_sk0_tumbler);
	bn_null(ec_sk1_tumbler);

	ec_null(ec_pk0_alice);
	ec_null(ec_pk1_alice);
	ec_null(ec_pk0_bob);
	ec_null(ec_pk1_bob);
	ec_null(ec_pk0_tumbler);
	ec_null(ec_pk1_tumbler);
	ec_null(ec_pk0_alice_tumbler);
	ec_null(ec_pk1_alice_tumbler);
	ec_null(ec_pk0_bob_tumbler);
	ec_null(ec_pk1_bob_tumbler);

	for (size_t i = 0; i < RING_SIZE; i++) {
		bn_null(ec_ring_m[i]);
		ec_null(ec_ring_pk_0[i]);
		ec_null(ec_ring_pk_1[i]);
	}
	...
	RLC_TRY {
		...
		bn_new(x);
		bn_new(y);
		bn_new(m_alice_tumbler);
		bn_new(m_bob_tumbler);
		bn_new(ec_sk0_alice);
		bn_new(ec_sk1_alice);
		bn_new(ec_sk0_bob);
		bn_new(ec_sk1_bob);
		bn_new(ec_sk0_tumbler);
		bn_new(ec_sk1_tumbler);

		ec_new(ec_pk0_alice);
		ec_new(ec_pk1_alice);
		ec_new(ec_pk0_bob);
		ec_new(ec_pk1_bob);
		ec_new(ec_pk0_tumbler);
		ec_new(ec_pk1_tumbler);
		ec_new(ec_pk0_alice_tumbler);
		ec_new(ec_pk1_alice_tumbler);
		ec_new(ec_pk0_bob_tumbler);
		ec_new(ec_pk1_bob_tumbler);

		for (size_t i = 0; i < RING_SIZE; i++) {
			bn_new(ec_ring_m[i]);
			ec_new(ec_ring_pk_0[i]);
			ec_new(ec_ring_pk_1[i]);
		}
		...
		ec_curve_get_ord(q);
		bn_rand_mod(ec_sk0_alice, q);
		bn_rand_mod(ec_sk1_alice, q);

		bn_rand_mod(ec_sk0_bob, q);
		bn_rand_mod(ec_sk1_bob, q);

		bn_rand_mod(ec_sk0_tumbler, q);
		bn_rand_mod(ec_sk1_tumbler, q);

		ec_mul_gen(ec_pk0_alice, ec_sk0_alice);
		ec_mul_gen(ec_pk1_alice, ec_sk1_alice);

		ec_mul_gen(ec_pk0_bob, ec_sk0_bob);
		ec_mul_gen(ec_pk1_bob, ec_sk1_bob);

		ec_mul_gen(ec_pk0_tumbler, ec_sk0_tumbler);
		ec_mul_gen(ec_pk1_tumbler, ec_sk1_tumbler);

		ec_add(ec_pk0_alice_tumbler, ec_pk0_alice, ec_pk0_tumbler);
		ec_norm(ec_pk0_alice_tumbler, ec_pk0_alice_tumbler);
		ec_add(ec_pk1_alice_tumbler, ec_pk1_alice, ec_pk1_tumbler);
		ec_norm(ec_pk1_alice_tumbler, ec_pk1_alice_tumbler);

		ec_add(ec_pk0_bob_tumbler, ec_pk0_bob, ec_pk0_tumbler);
		ec_norm(ec_pk0_bob_tumbler, ec_pk0_bob_tumbler);
		ec_add(ec_pk1_bob_tumbler, ec_pk1_bob, ec_pk1_tumbler);
		ec_norm(ec_pk1_bob_tumbler, ec_pk1_bob_tumbler);

		// Generate random ring public keys and bitstrings m_i.
		bn_rand_mod(m_alice_tumbler, q);
		bn_rand_mod(m_bob_tumbler, q);

		for (size_t i = 0; i < RING_SIZE; i++) {
			bn_rand_mod(ec_ring_m[i], q);
			ec_rand(ec_ring_pk_0[i]);
			ec_rand(ec_ring_pk_1[i]);
		}
		...
		bn_write_bin(serialized_ec_sk, RLC_BN_SIZE, ec_sk0_alice);
		fwrite(serialized_ec_sk, sizeof(uint8_t), RLC_BN_SIZE, file);
		ec_write_bin(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED, ec_pk0_alice_tumbler, 1);
		fwrite(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file);

		memzero(serialized_ec_sk, RLC_BN_SIZE);
		bn_write_bin(serialized_ec_sk, RLC_BN_SIZE, ec_sk1_alice);
		fwrite(serialized_ec_sk, sizeof(uint8_t), RLC_BN_SIZE, file);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		ec_write_bin(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED, ec_pk1_alice_tumbler, 1);
		fwrite(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file);

		bn_write_bin(serialized_m, RLC_BN_SIZE, m_alice_tumbler);
		fwrite(serialized_m, sizeof(uint8_t), RLC_BN_SIZE, file);
    ...
    fwrite(GENtostr(cl_pk_alice), sizeof(char), RLC_CL_PUBLIC_KEY_SIZE, file);
		...
		for (size_t i = 0; i < RING_SIZE; i++) {
			memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
			ec_write_bin(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED, ec_ring_pk_0[i], 1);
			fwrite(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file);

			memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
			ec_write_bin(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED, ec_ring_pk_1[i], 1);
			fwrite(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file);
			
			memzero(serialized_m, RLC_BN_SIZE);
			bn_write_bin(serialized_m, RLC_BN_SIZE, ec_ring_m[i]);
			fwrite(serialized_m, sizeof(uint8_t), RLC_BN_SIZE, file);
		}
		...
		bn_write_bin(serialized_ec_sk, RLC_BN_SIZE, ec_sk0_bob);
		fwrite(serialized_ec_sk, sizeof(uint8_t), RLC_BN_SIZE, file);
		ec_write_bin(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED, ec_pk0_bob_tumbler, 1);
		fwrite(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file);

		memzero(serialized_ec_sk, RLC_BN_SIZE);
		bn_write_bin(serialized_ec_sk, RLC_BN_SIZE, ec_sk1_bob);
		fwrite(serialized_ec_sk, sizeof(uint8_t), RLC_BN_SIZE, file);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		ec_write_bin(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED, ec_pk1_bob_tumbler, 1);
		fwrite(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file);

		bn_write_bin(serialized_m, RLC_BN_SIZE, m_bob_tumbler);
		fwrite(serialized_m, sizeof(uint8_t), RLC_BN_SIZE, file);
    ...
    fwrite(GENtostr(cl_pk_bob), sizeof(char), RLC_CL_PUBLIC_KEY_SIZE, file);
		...
		// Tumbler has four EC public keys, two with Alice and two with Bob.
		bn_write_bin(serialized_ec_sk, RLC_BN_SIZE, ec_sk0_tumbler);
		fwrite(serialized_ec_sk, sizeof(uint8_t), RLC_BN_SIZE, file);
		ec_write_bin(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED, ec_pk0_alice_tumbler, 1);
		fwrite(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		...
		memzero(serialized_ec_sk, RLC_BN_SIZE);
		bn_write_bin(serialized_ec_sk, RLC_BN_SIZE, ec_sk1_tumbler);
		fwrite(serialized_ec_sk, sizeof(uint8_t), RLC_BN_SIZE, file);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		ec_write_bin(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED, ec_pk1_alice_tumbler, 1);
		fwrite(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file);
    ...
    fwrite(GENtostr(cl_pk_tumbler), sizeof(char), RLC_CL_PUBLIC_KEY_SIZE, file);
		...
		bn_free(x);
		bn_free(y);
		bn_free(m_alice_tumbler);
		bn_free(m_bob_tumbler);
		bn_free(ec_sk0_alice);
		bn_free(ec_sk1_alice);
		bn_free(ec_sk0_bob);
		bn_free(ec_sk1_bob);
		bn_free(ec_sk0_tumbler);
		bn_free(ec_sk1_tumbler);
		
		ec_free(ec_pk0_alice);
		ec_free(ec_pk1_alice);
		ec_free(ec_pk0_bob);
		ec_free(ec_pk1_bob);
		ec_free(ec_pk0_tumbler);
		ec_free(ec_pk1_tumbler);
		ec_free(ec_pk0_alice_tumbler);
		ec_free(ec_pk1_alice_tumbler);
		ec_free(ec_pk0_bob_tumbler);
		ec_free(ec_pk1_bob_tumbler);

		for (size_t i = 0; i < RING_SIZE; i++) {
			bn_free(ec_ring_m[i]);
			ec_free(ec_ring_pk_0[i]);
			ec_free(ec_ring_pk_1[i]);
		}
...
int read_keys_from_file_alice_bob(const char *name,
	...
	RLC_TRY {
		...
		bn_read_bin(keys->ec_sk0->sk, serialized_ec_sk, RLC_BN_SIZE);
		memzero(serialized_ec_sk, RLC_BN_SIZE);

		if (fread(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file) != RLC_EC_SIZE_COMPRESSED) {
			RLC_THROW(ERR_NO_READ);
		}
		ec_read_bin(keys->ec_pk0->pk, serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);

		if (fread(serialized_ec_sk, sizeof(uint8_t), RLC_BN_SIZE, file) != RLC_BN_SIZE) {
			RLC_THROW(ERR_NO_READ);
		}
		bn_read_bin(keys->ec_sk1->sk, serialized_ec_sk, RLC_BN_SIZE);

		if (fread(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file) != RLC_EC_SIZE_COMPRESSED) {
			RLC_THROW(ERR_NO_READ);
		}
		ec_read_bin(keys->ec_pk1->pk, serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);

		if (fread(serialized_m, sizeof(uint8_t), RLC_BN_SIZE, file) != RLC_BN_SIZE) {
			RLC_THROW(ERR_NO_READ);
		}
		bn_read_bin(keys->m, serialized_m, RLC_BN_SIZE);
		...
		for (size_t i = 0; i < RING_SIZE; i++) {
			...
			ec_read_bin(ring->pk_0[i], serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);

			memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
			if (fread(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file) != RLC_EC_SIZE_COMPRESSED) {
				RLC_THROW(ERR_NO_READ);
			}
			ec_read_bin(ring->pk_1[i], serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);

			memzero(serialized_m, RLC_BN_SIZE);
			if (fread(serialized_m, sizeof(uint8_t), RLC_BN_SIZE, file) != RLC_BN_SIZE) {
				RLC_THROW(ERR_NO_READ);
			}
			bn_read_bin(ring->m[i], serialized_m, RLC_BN_SIZE);
		}
		...
		fseek(file, (4 * RLC_BN_SIZE) + (4 * RLC_EC_SIZE_COMPRESSED) + RLC_CL_SECRET_KEY_SIZE, SEEK_SET);
		if (fread(serialized_cl_pk, sizeof(char), RLC_CL_PUBLIC_KEY_SIZE, file) != RLC_CL_PUBLIC_KEY_SIZE) {
			RLC_THROW(ERR_NO_READ);
		}
		tumbler_cl_public_key->pk = gp_read_str(serialized_cl_pk);

		fseek(file, (RING_SIZE * ((2 * RLC_EC_SIZE_COMPRESSED) + RLC_BN_SIZE)) + (2 * RLC_CL_PUBLIC_KEY_SIZE) 
				+ RLC_G1_SIZE_COMPRESSED, SEEK_CUR);
...
int read_keys_from_file_tumbler(keys_t keys_alice,
	...
	RLC_TRY {
		...
		bn_read_bin(keys_alice->ec_sk0->sk, serialized_ec_sk, RLC_BN_SIZE);
		bn_read_bin(keys_bob->ec_sk0->sk, serialized_ec_sk, RLC_BN_SIZE);
		memzero(serialized_ec_sk, RLC_BN_SIZE);

		if (fread(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file) != RLC_EC_SIZE_COMPRESSED) {
			RLC_THROW(ERR_NO_READ);
		}
		ec_read_bin(keys_alice->ec_pk0->pk, serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		
		if (fread(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file) != RLC_EC_SIZE_COMPRESSED) {
			RLC_THROW(ERR_NO_READ);
		}
		ec_read_bin(keys_bob->ec_pk0->pk, serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);

		if (fread(serialized_ec_sk, sizeof(uint8_t), RLC_BN_SIZE, file) != RLC_BN_SIZE) {
			RLC_THROW(ERR_NO_READ);
		}
		bn_read_bin(keys_alice->ec_sk1->sk, serialized_ec_sk, RLC_BN_SIZE);
		bn_read_bin(keys_bob->ec_sk1->sk, serialized_ec_sk, RLC_BN_SIZE);
		memzero(serialized_ec_sk, RLC_BN_SIZE);

		if (fread(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file) != RLC_EC_SIZE_COMPRESSED) {
			RLC_THROW(ERR_NO_READ);
		}
		ec_read_bin(keys_alice->ec_pk1->pk, serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		
		if (fread(serialized_ec_pk, sizeof(uint8_t), RLC_EC_SIZE_COMPRESSED, file) != RLC_EC_SIZE_COMPRESSED) {
			RLC_THROW(ERR_NO_READ);
		}
		ec_read_bin(keys_bob->ec_pk1->pk, serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);
		memzero(serialized_ec_pk, RLC_EC_SIZE_COMPRESSED);

		if (fread(serialized_m, sizeof(uint8_t), RLC_BN_SIZE, file) != RLC_BN_SIZE) {
			RLC_THROW(ERR_NO_READ);
		}
		bn_read_bin(keys_alice->m, serialized_m, RLC_BN_SIZE);
		memzero(serialized_m, RLC_BN_SIZE);

		if (fread(serialized_m, sizeof(uint8_t), RLC_BN_SIZE, file) != RLC_BN_SIZE) {
			RLC_THROW(ERR_NO_READ);
		}
		bn_read_bin(keys_bob->m, serialized_m, RLC_BN_SIZE);
