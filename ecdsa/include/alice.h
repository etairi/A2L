#ifndef TRILERO_ECDSA_INCLUDE_ALICE
#define TRILERO_ECDSA_INCLUDE_ALICE
...
#define alice_state_new(state)                              \
  do {                                                      \
    ...
    bn_new((state)->ctx_alpha_times_beta);                  \
    bn_new((state)->k_1);                                   \
    ec_new((state)->R_1);                                   \
    bn_new((state)->s_hat);                                 \
    bn_new((state)->s);                                     \
...
#define alice_state_free(state)                             \
  do {                                                      \
    ...
    bn_free((state)->ctx_alpha_times_beta);                 \
    bn_free((state)->k_1);                                  \
    ec_free((state)->R_1);                                  \
    bn_free((state)->s_hat);                                \
    bn_free((state)->s);                                    \
