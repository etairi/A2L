...
int puzzle_share_handler(alice_state_t state, void *socket, uint8_t *data) {
  ...
  TRY {
    ...
    ec_read_bin(state->g_to_the_alpha_plus_beta, data, RLC_EC_SIZE_COMPRESSED);
    bn_read_bin(state->ctx_alpha_plus_beta, data + RLC_EC_SIZE_COMPRESSED, RLC_PAILLIER_CTX_SIZE);
...
int payment_init_done_handler(alice_state_t state, void *socket, uint8_t *data) {
  ...
  TRY {
    ...
    bn_write_bin(in, in_len, state->tau);
		if (cp_phpe_enc(out, &out_len, in, in_len, state->tumbler_paillier_pk->pk) != RLC_OK) {
      THROW(ERR_CAUGHT);
    }
    bn_read_bin(ctx_tau, out, out_len);

    bn_sqr(s, state->tumbler_paillier_pk->pk);
		bn_mul(ctx_alpha_plus_beta_plus_tau, state->ctx_alpha_plus_beta, ctx_tau);
		bn_mod(ctx_alpha_plus_beta_plus_tau, ctx_alpha_plus_beta_plus_tau, s);
    ...
    const unsigned msg_type_length = (unsigned) strlen(msg_type) + 1;
    const unsigned msg_data_length = (2 * RLC_EC_SIZE_COMPRESSED) + RLC_BN_SIZE + RLC_PAILLIER_CTX_SIZE;
    const int total_msg_length = msg_type_length + msg_data_length + (2 * sizeof(unsigned));
    ...
    bn_write_bin(payment_sign_msg->data + (2 * RLC_EC_SIZE_COMPRESSED) + RLC_BN_SIZE, RLC_PAILLIER_CTX_SIZE, ctx_alpha_plus_beta_plus_tau);
...
int payment_sign_done_handler(alice_state_t state, void *socket, uint8_t *data) {
  ...
  ec_t g_to_the_tau, A_prime_time_g_to_the_tau;
  ...
  ec_null(g_to_the_tau);
  ec_null(A_prime_time_g_to_the_tau);
  ...
  TRY {
    ...
    ec_new(g_to_the_tau);
    ec_new(A_prime_time_g_to_the_tau);
    ec_new(g_to_the_s_2);
    ...
    // Sanity check.
    ec_mul_gen(g_to_the_tau, state->tau);
    ec_add(A_prime_time_g_to_the_tau, state->g_to_the_alpha_plus_beta, g_to_the_tau);
    ec_norm(A_prime_time_g_to_the_tau, A_prime_time_g_to_the_tau);
    if (ec_cmp(A_prime_time_g_to_the_tau, g_to_the_gamma) != RLC_EQ) {
      THROW(ERR_CAUGHT);
    }
    ...
    ec_free(g_to_the_tau);
    ec_free(A_prime_time_g_to_the_tau);
...
int puzzle_solve_handler(alice_state_t state, void *socket, uint8_t *data) {
  ...
  TRY {
    ...
    bn_sub(state->alpha_hat, gamma, state->tau);
